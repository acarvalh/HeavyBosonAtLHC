...
expected.write_txt('results/ATLAS_VV_llJ_WZ_ourfit_Fudge_expected.txt')
observed.write_txt('results/ATLAS_VV_llJ_WZ_ourfit_Fudge_observed.txt')
#zlevel.write_txt('CMS_ATLAS_VV_JJ_WZ_ourfit_noFudge_zlevel.txt')


current = 1
with open('results/ATLAS_VV_llJ_WZ_ourfit_Fudge_zlevel.txt', 'w') as fff:
    while current < 11:
        masse = mass[current]
        sig='WZ'+str(masse)
        zl = zlevel[sig]['Z'][0]
...
# ../theta/utils2/theta-auto.py analysis_ATLAS_WZ_llJ_ourfit_fudge.py